...
def test_labels_len_must_match() -> None:
    with pytest.raises(ShapeError):
...
def test_select_dims_out_of_range() -> None:
...
def test_from_csv_with_index(tmp_path: Path) -> None:
    import csv
    p = tmp_path / "d.csv"
...
def test_from_csv_missing_file() -> None:
    with pytest.raises(DataError):
