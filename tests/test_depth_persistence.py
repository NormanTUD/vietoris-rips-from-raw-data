...
A: frozenset[int] = frozenset({0, 1, 2})
A2: frozenset[int] = frozenset({0, 1, 3})
B: frozenset[int] = frozenset({10, 11, 12})


def test_chain_consecutive_persistence() -> None:
...
def test_chain_two_separate_features() -> None:
    chains = _match_chains({0: [A], 1: [A], 2: [B]})
    spans = sorted(c.span for c in chains)
    assert spans == [(0, 1), (2, 2)]


def test_chain_gap_allowed_when_max_gap_big() -> None:
...
def test_chain_gap_blocked_when_max_gap_small() -> None:
...
def test_chain_no_match_below_threshold() -> None:
...
def test_stable_core_filters_short_chains() -> None:
    chains = [
...
def test_betti_heatmap_torus_essential() -> None:
...
def test_betti_heatmap_betti_and_persistence_metrics() -> None:
    C = make_torus_grid_complex(2, (3, 3))
    bc = persistent_homology(C)
    lr = LayerResult(layer=0, complex=C, barcode=bc, nn=1.0, eps_max=float(C.values.max()),
                     labels=[str(i) for i in range(9)])
...
def test_depth_chains_smoke_on_real_data() -> None:
...
def _synthetic_layer_results() -> dict[int, LayerResult]:
...
def test_synthetic_loop_persists_across_layers() -> None:
...
def test_synthetic_loop_absent_in_middle_layer_breaks_at_gap1() -> None:
...
def test_depth_profile_reports_activity() -> None:
    lr = _synthetic_layer_results()
