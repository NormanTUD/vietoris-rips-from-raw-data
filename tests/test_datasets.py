...
def test_load_token_cloud_shape():
...
def test_load_token_cloud_dim_selection():
    ps = datasets.load_token_cloud(layer=0).select_dims([0, 1, 2])
    assert ps.dim == 3


def test_load_layer_points():
...
def test_load_residual_matrix_norms():
...
def test_load_residual_matrix_kinds():
...
def test_load_residual_matrix_bad_kind():
...
def test_token_cloud_is_finite():
