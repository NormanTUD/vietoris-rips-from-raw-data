...
def test_landscape_monotone_in_level():
...
def test_landscape_empty():
    xgrid, F = M.persistence_landscape(Barcode(intervals=[]), dim=1)
    assert F.max() == 0.0


def test_image_identical_barcodes_identical():
    def b():
...
def test_image_mass_near_point():
