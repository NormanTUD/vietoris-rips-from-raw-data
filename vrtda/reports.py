...
class Report:
    ...
    def table(self, heading: str, headers: list[str], rows: list[list]) -> "Report":
        def cell(v):
    ...
    def write(self, path, fmt: str = "md") -> Path:
        path = Path(path)
...
def betti_table(epsilons, betti_arr) -> tuple[list[str], list[list]]:
